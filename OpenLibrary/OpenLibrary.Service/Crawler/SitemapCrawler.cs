...
using OpenLibrary.Service.Crawler.Interface;
...
namespace OpenLibrary.Service.Crawler
{
    /// <summary>
    ...
    public class SitemapCrawler : IWebCrawler<Sitemap>
    {
        ...
        private void RunThread_Report(UrlWebService service, bool success)
        {
            if (success)
            {
                OnServiceMessage(String.Format("Web service succeess:  {0}", service.Url));
            ...
            {
                OnServiceMessage(String.Format("Web service failed:  {0}", service.Url));
        ...
        private IEnumerable<Sitemap> RunThread_TransformResult(IWebService<string> service)
        {
            ...
            {
                OnServiceError("Error reading sitemap:  " + service.Url, ex);
